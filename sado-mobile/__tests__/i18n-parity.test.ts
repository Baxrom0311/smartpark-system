 ...
 *
 *   1. **Parity** — every key path that exists in `uz.json` must also
 *      exist in `ru.json` (and vice versa). Missing translations are
 *      easy to introduce by editing only one resource file, and the
 *      result is a key string leaking into the UI in the other
 *      language. This guard catches that on CI before merge.
 *
 *   2. **Hardcoded strings** — any JSX text node in `app/**/*.tsx`
 *      that isn't a `t(...)` call, an expression, or an allow-listed
 *      brand token. Anchoring the regex to `>...<` JSX text content
 *      (and stripping comments first) keeps false-positive noise out
 *      while still catching real violations like `<Text>Hello</Text>`.
 *
 *      If a legitimate non-translated literal is needed (brand names,
 *      single punctuation, language toggle codes), add it to
 *      {@link STRING_ALLOWLIST} below — never silence the regex.
 */
...
/**
 * Strip block (`/* ... *\/`) and line (`// ...`) comments from a TS
 * source so the JSX-text regex never matches operators inside doc
 * comments (e.g. `// returns >0`).
 *
...
function stripComments(source: string): string {
  // Block comments first so a `//` inside `/* */` doesn't trigger the
  // line-comment branch.
 ...
 * NOT contain JSX-meta or TS/JS punctuation (`<`, `>`, `{`, `}`,
 * `(`, `)`, `[`, `]`, `;`, `=`, `\`, `:`, `,`), then `<`. Anything
 * with TS/JS punctuation can't be JSX text in practice and is
 * almost certainly an operator (`=>`, `<T>`, comparisons).
 *
...
describe("hardcoded-string guard (app/**/*.tsx)", () => {
